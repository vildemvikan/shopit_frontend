...
vi.mock('@/router', () => {
  ...
  const mockedRouter = createRouter({
    ...
    routes: [
      ...
      { path: '/advertisement/:id', name: 'Advertisement', component: { template: '<div>Ad</div>' } },
...
describe('Schema.vue', () => {
  ...
  it('initializes internal state from the advertisement store when new is true (manual store override)', async () => {
    ...
    advertisementStore.images = [{ url: 'http://example.com/store.jpg', caption: 'Store image', publicId: null }];
    ...
    expect(vm.images).toEqual([{ url: 'http://example.com/store.jpg', caption: 'Store image', publicId: null }]);
