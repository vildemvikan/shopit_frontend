...
describe('ResetPasswordForm.vue', () => {
  ...
  it('resets password successfully when valid credentials are provided', async () => {
    ...
    expect(resetPasswordWithToken).toHaveBeenCalledWith('test-token', 'test@example.com', 'password123');
  ...
  it('shows invalid token message when token is invalid', async () => {
    ...
    expect(wrapper.find('.invalid-token-message').text()).toContain(i18n.global.t('invalidOrExpiredTokenTitle'));
  });
});
